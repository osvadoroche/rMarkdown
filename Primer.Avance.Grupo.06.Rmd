...
/*Este Codigo se utiliza unicamente para dar estilo al documento*/
...
library(dplyr)
library(readxl)
Data_Trabajo <- read_excel("../Data/Data_Trabajo.xlsx")
Data_Trabajo <- Data_Trabajo %>%
  rename(Exportaciones.Camisas=Xc,Tasa.Apertura=TA,
         Pib.Per.Capita=Ypc,
         Precios.Exportaciones=IPX,
         Importacion.Capital=Mk,
         Produccion.Industrial=PI)
Data_Trabajo <- as.data.frame(Data_Trabajo)
Data_Trabajo$Pib.Per.Capita <- as.numeric(Data_Trabajo$Pib.Per.Capita)
Data_Trabajo$Precios.Exportaciones <- as.numeric(Data_Trabajo$Precios.Exportaciones)
Data_Trabajo$Produccion.Industrial <- as.numeric(Data_Trabajo$Produccion.Industrial)
...
Ecuacion_Modelo <- lm(formula = Exportaciones.Camisas ~ Tasa.Apertura +
                        Pib.Per.Capita+Precios.Exportaciones+
                        Importacion.Capital+Produccion.Industrial,
                        data = Data_Trabajo)
stargazer(Ecuacion_Modelo,
          title = "Ecuación Estimada", 
          type = "text", 
          digits = 4 )
summary(Ecuacion_Modelo)
```

## 12.2. Pruebas de Distribución Normal
...
## 12.3. Prueba de Jarque Bera

```{r}
jb.norm.test(Ecuacion_Modelo$residuals)
```

## 12.4. Prueba de Kolmogorov-Smirnov

```{r}
lillie.test(Ecuacion_Modelo$residuals)
```

## 12.5. Prueba de Shapiro Wilk

```{r}
shapiro.test(Ecuacion_Modelo$residuals)
```

# 13. Pruebas de Multicolinealidad
...
# Factores Inflasionarios de la Varianza
